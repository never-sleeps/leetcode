package com.leetcode.Baseball_Game;
    ...
    public int calPoints(String[] ops) {
        ...
        for(String op : ops) {
            switch (op) {
                // "+" - Record a new score that is the sum of the previous two scores.
                case "+" -> {
                    int top = stack.pop(); // pop() вытащит и удалит верхний элемент очереди
                    int newTop = top + stack.peek(); // peek() - посмотреть на элемент в начале очереди, не вынимая его из очереди
                    stack.push(top); // push(obj) - Добавляет элемент obj в начало очереди
                    stack.push(newTop);
                }
                // "C" - Invalidate the previous score, removing it from the record.
                case "C" -> stack.pop();
...
class Main {
    public static void main(String[] args) {
        var result = new Solution().calPoints(new String[] {"5","2","C","D","+"});
        assert result == 30;
    }
}